...
class RealDiscriminantStrategy implements DiscriminantStrategy
{
  ...
  {
    double result = b * b - (4 * a * c);
    result = result < 0 ? Double.NaN : result;
    return result;
  }
}
...
class QuadraticEquationSolver
{
  ...
  public Pair<Complex, Complex> solve(double a, double b, double c)
  {
    ...
    // double negativeDiscrimant = discriminant < 0 ? -discriminant : discriminant;
    Complex dis = Complex.sqrt(discriminant);
    Complex denom = new Complex(a, 0).times(new Complex(2,0));
    Complex sol1 = new Complex(-b, 0).plus(dis).divides(denom);
    Complex sol2 = new Complex(-b, 0).minus(dis).divides(denom);
    Pair<Complex, Complex> pair = new Pair<>(sol1, sol2);
    return pair;
  }
}
...
class Demo {
  public static void main(String[] args) {
    DiscriminantStrategy ds = new OrdinaryDiscriminantStrategy();
    QuadraticEquationSolver qes = new QuadraticEquationSolver(ds);
    Pair<Complex, Complex> pair = qes.solve(5, 2, 1);
